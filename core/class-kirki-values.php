...
class Xirki_Values {
	...
	public function __construct() {
		add_filter( 'xirki_values_get_value', array( $this, 'typography_field_tweaks' ), 10, 2 );
	}
	...
	public function typography_field_tweaks( $value, $field_id ) {
		if ( isset( Xirki::$fields[ $field_id ] ) && isset( Xirki::$fields[ $field_id ]['type'] ) ) {
			if ( 'xirki-typography' === Xirki::$fields[ $field_id ]['type'] ) {
	 ...
	 * @static
	 * @access public
	 * @param string $config_id The configuration ID. @see Xirki_Config.
	 * @param string $field_id  The field ID.
	 * @return string|array
	 */
	public static function get_value( $config_id = '', $field_id = '' ) {
		...
		if ( ! isset( Xirki::$config[ $config_id ] ) ) {
			$config_id = 'global';
		}

		if ( 'theme_mod' === Xirki::$config[ $config_id ]['option_type'] ) {

			// We're using theme_mods so just get the value using get_theme_mod.
			$default_value = null;
			if ( isset( Xirki::$fields[ $field_id ] ) && isset( Xirki::$fields[ $field_id ]['default'] ) ) {
				$default_value = Xirki::$fields[ $field_id ]['default'];
			}
			$value = get_theme_mod( $field_id, $default_value );
			return apply_filters( 'xirki_values_get_value', $value, $field_id );
		}

		if ( 'option' === Xirki::$config[ $config_id ]['option_type'] ) {

			// We're using options.
			if ( '' !== Xirki::$config[ $config_id ]['option_name'] ) {

				// Options are serialized as a single option in the db.
				// We'll have to get the option and then get the item from the array.
				$options = get_option( Xirki::$config[ $config_id ]['option_name'] );

				if ( ! isset( Xirki::$fields[ $field_id ] ) && isset( Xirki::$fields[ Xirki::$config[ $config_id ]['option_name'] . '[' . $field_id . ']' ] ) ) {
					$field_id = Xirki::$config[ $config_id ]['option_name'] . '[' . $field_id . ']';
				}
				$setting_modified = str_replace( ']', '', str_replace( Xirki::$config[ $config_id ]['option_name'] . '[', '', $field_id ) );

				$default_value = ( isset( Xirki::$fields[ $field_id ] ) && isset( Xirki::$fields[ $field_id ]['default'] ) ) ? Xirki::$fields[ $field_id ]['default'] : '';
			...
			$value = get_option( $field_id, Xirki::$fields[ $field_id ]['default'] );
			return apply_filters( 'xirki_values_get_value', $value, $field_id );
