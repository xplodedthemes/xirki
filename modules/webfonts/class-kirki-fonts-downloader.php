...
class Xirki_Fonts_Downloader {
	...
	protected function get_local_files_from_css( $css ) {
		...
		$stored     = get_option( 'xirki_downloaded_font_files', array() );
		...
		if ( $change ) {
			update_option( 'xirki_downloaded_font_files', $stored );
		}
	...
	public function get_cached_url_contents( $url = '', $user_agent = null ) {
		...
		$contents         = false;
		$cached_responses = get_transient( 'xirki_remote_url_contents' );
		...
		if ( $contents ) {
			...
			set_transient( 'xirki_remote_url_contents', $cached_responses, WEEK_IN_SECONDS );
		}
